...
// /workshops = 숙의 운영자 화면(목록/생성/콘솔/설정/프로젝터). /dialogue = 대화 렌즈 운영자/진행자 화면.
// 서버컴포넌트가 adminContext 로 직접 읽으므로
// 미들웨어 게이트가 없으면 무인증 노출된다 (C1). 참가자 화면은 /p/* 로 분리되어 있어 영향 없음.
const PROTECTED_PREFIXES = ['/today', '/companies', '/courses', '/dates', '/settings', '/workshops', '/dialogue']
