...
import { exportSessionExcelBuffer } from '@/lib/export/excel'
...
export async function GET(req: Request) {
  ...
  try {
    const buf = await exportSessionExcelBuffer(session.id)
    ...
    return NextResponse.json({ ok: false, error: e instanceof Error ? e.message : 'xlsx error' }, { status: 500 })
  }
}
