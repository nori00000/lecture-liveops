...
- `라이브 전사 렌즈`가 첫 번째 운영 패널로 보인다.
  - 실제 전사 segment가 있으면 `전사 연결됨`으로 표시되고 최근 전사 흐름, 키워드, 그룹 발화량, 질문/동의/반대/우려 신호가 갱신된다.
  - 전사 수집 파이프라인이 아직 연결되지 않은 데모/개발 환경에서는 `전사 미연결 프리뷰`로 표시되고, 참가자 제출 발언을 전사 흐름처럼 시각화한다.
...
- `operator-console-top`: 라이브 전사 렌즈, 라운드 제어, 프로젝터 발행, 리포트 다운로드.
...
9. `라이브 전사 렌즈`에서 키워드와 그룹 발화량이 현장 감각과 맞는지 본다. `전사 미연결 프리뷰`라면 제출 발언 기반 시각화임을 운영자끼리 공유한다.
10. 필요한 경우 moderation 큐에서 신고/숨김/복원을 처리한다.
11. 종합 시점에 `결과 계산 & 발행`을 실행한다.
12. 프로젝터 결과판을 보고 합의점·쟁점·소수 관점을 설명한다.
13. 세션 종료 후 `검토 후보 생성`을 실행하고, 필요한 후보만 승인한다.
14. Markdown, HTML, Excel 리포트를 내려받아 납품한다.

## 4. 납품 전 검수 체크리스트

- [ ] 프로젝터 결과판에 공감대·쟁점·소수 관점이 표시된다.
- [ ] 운영자 콘솔 상단에 `라이브 전사 렌즈`가 표시되고, 현재 세션이 `전사 연결됨`인지 `전사 미연결 프리뷰`인지 구분된다.
...
### 라이브 전사 렌즈가 `전사 미연결 프리뷰`로 보인다

정상일 수 있다. 현재 소프트웨어는 전사 테이블을 읽어 시각화하는 경로와 데모 프리뷰를 제공하지만, 실제 마이크/WhisperLive ingest 파이프라인은 아직 연결하지 않았다. 이 상태에서는 참가자 제출 발언을 기반으로 키워드와 그룹 활동을 보여준다.
